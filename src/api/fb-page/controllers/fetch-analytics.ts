...
import facebookGraphAPI from '../services/facebook-graph-api'
import { fetchPageReactions } from '../services/facebook-reaction-api';
...
export default factories.createCoreController('api::fb-page.fb-page', ({ strapi }) => ({
  async fetchAnalytics(ctx) {
    ...
    try {
      ...
      const analyticsResponse = await facebookGraphAPI.getAnalyticsData(
        facebook_page_id, 
        adminUser.access_token, 
        date
      );

      const growthResponse = await facebookGraphAPI.getAudienceGrowthData(
        facebook_page_id, 
        adminUser.access_token, 
        date
      );
      ...
      try {
        reactionResponse = await fetchPageReactions(facebook_page_id, adminUser.access_token, date);
        reactionData = reactionResponse.data?.find((m: any) => m.name === 'page_actions_post_reactions_total');
      } catch (err) {
        console.error('Error fetching Facebook reactions:', err?.response?.data || err.message || err);
      ...
      if (!existingAnalytics) {
        const analyticsEntry = {
          fb_page: page.id,
          date,
          impressions: facebookGraphAPI.extractMetricValue(analyticsData, 'page_impressions'),
          views: facebookGraphAPI.extractMetricValue(analyticsData, 'page_views_total'),
          reach: facebookGraphAPI.extractMetricValue(analyticsData, 'page_impressions_unique'),
          followers: facebookGraphAPI.extractMetricValue(analyticsData, 'page_fans'),
          engagements: facebookGraphAPI.extractMetricValue(analyticsData, 'page_post_engagements'),
        };
      ...
      if (!existingGrowth) {
        const growthEntry = {
          fb_page: page.id,
          date,
          followers: facebookGraphAPI.extractMetricValue(growthData, 'page_fans'),
          new_followers: facebookGraphAPI.extractMetricValue(growthData, 'page_fan_adds'),
        };
      ...
      if (analyticsCreated) createdItems.push('analytics');
      else skippedItems.push('analytics');
      
      if (growthCreated) createdItems.push('audience growth');
      else skippedItems.push('audience growth');
